...
addCommandHandler("arrest", function(player, _, playerIdOrName, time, ...)
    ...
    if arrestPlayer(player, target, reason, time) then
        sendMessageToTeam(getPlayerTeam(player), COLOR_BLUE .. getPlayerName(player) .. " arrested " ..
            getPlayerName(target) .. " (" .. reason .. ")")
    end
end)
...
addCommandHandler("ticket", function(player, _, playerIdOrName, amount, ...)
    ...
    outputChatBox(COLOR_ORANGE .. "You received a ticket of $" .. formatMoney(amount) .. " from " ..
                      getPlayerName(player), target)
    outputChatBox(COLOR_ORANGE .. "Reason: " .. reason, target)
    outputChatBox(COLOR_GREEN .. "You issued a $" .. formatMoney(amount) .. " ticket to " .. getPlayerName(target),
        player)
...
addCommandHandler("backup", function(player, _, ...)
    ...
    local backupMsg = COLOR_RED .. "[BACKUP REQUESTED] " .. COLOR_WHITE .. getPlayerName(player) .. " at " .. zone ..
                          ": " .. message
