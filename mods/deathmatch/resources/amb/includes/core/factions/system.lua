...
addCommandHandler("createfaction", function(player, cmd, factionName, ...)
    ...
    outputChatBox(string.format("✅ Da tao faction '%s' (ID: %d, Type: %s)", factionName, factionID, factionType),
        player, 0, 255, 0)
    outputChatBox(string.format("🏢 FACTION CREATED: %s [%s] by %s", factionName, factionType, getPlayerName(player)),
        root, 255, 255, 0)
end)
...
addCommandHandler("invitef", function(player, cmd, targetName, factionID)
    ...
    outputChatBox(string.format("📨 Da gui loi moi vao faction '%s' cho %s.", factionData.name,
        getPlayerName(targetPlayer)), player, 0, 255, 0)
    outputChatBox(string.format("📨 %s moi ban vao faction '%s'. Su dung /acceptf hoac /declinef",
        getPlayerName(player), factionData.name), targetPlayer, 255, 255, 0)
end)
...
addCommandHandler("kickf", function(player, cmd, targetName)
    ...
    for _, p in ipairs(getElementsByType("player")) do
        ...
        if pData.faction == playerData.faction and p ~= player and p ~= targetPlayer then
            outputChatBox(
                string.format("🏢 %s da bi kick boi %s.", getPlayerName(targetPlayer), getPlayerName(player)), p, 255,
                255, 100)
        end
    end
end)
...
addCommandHandler("setrank", function(player, cmd, targetName, rank)
    ...
    outputChatBox(string.format("✅ Da set rank %s cho %s (Rank %d - %s).", newRank, getPlayerName(targetPlayer),
        newRank, rankNames[newRank]), player, 0, 255, 0)
    outputChatBox(string.format("📈 Rank cua ban da duoc set thanh %d (%s).", newRank, rankNames[newRank]),
...
addCommandHandler("fonline", function(player)
    ...
    if #onlineMembers == 0 then
        ...
        for _, member in ipairs(onlineMembers) do
            outputChatBox(string.format("• %s - %s (Rank %d)", member.name, rankNames[member.rank] or "Unknown",
                member.rank), player, 255, 255, 255)
        end
    end
...
addCommandHandler("f", function(player, cmd, ...)
    ...
    local chatMessage =
        string.format("(( [%s] %s %s: %s ))", factionData.name, rankName, getPlayerName(player), message)
...
addCommandHandler("transferleadership", function(player, cmd, targetName)
    ...
    for _, p in ipairs(getElementsByType("player")) do
        local pData = getElementData(p, "playerData") or {}
        if pData.faction == playerData.faction and p ~= player and p ~= targetPlayer then
            outputChatBox(string.format("🏢 %s da transfer leadership cho %s.", getPlayerName(player),
                getPlayerName(targetPlayer)), p, 255, 255, 0)
        end
    end
end)
