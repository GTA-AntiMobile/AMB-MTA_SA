...
addCommandHandler("dealership", function(player, cmd)
    ...
    for id, dealership in pairs(dealerships) do
        local distance = getDistanceBetweenPoints3D(x, y, z, dealership.location.x, dealership.location.y,
            dealership.location.z)
        if distance < 50 then
...
addCommandHandler("dealerships", function(player, cmd)
    ...
    for id, dealership in pairs(dealerships) do
        ...
        outputChatBox("   Vi tri: " .. math.floor(dealership.location.x) .. ", " .. math.floor(dealership.location.y),
            player, 200, 200, 200)
...
addCommandHandler("buyvehicle", function(player, _, vehicleIndex)
    ...
    if newVehicle then
        ...
        outputChatBox("Chuc mung! Ban da mua " .. vehicle.name .. " voi gia $" .. formatMoney(vehicle.price), player, 0,
            255, 0)
...
addCommandHandler("vinfo", function(player)
    local vehicle = getPedOccupiedVehicle(player)
    if not vehicle then
        ...
        if not nearestVeh then
            outputChatBox("❌ Khong co xe nao gan day.", player, 255, 100, 100)
            return
        end
    ...
    outputChatBox("🚗 ===== THONG TIN XE =====", player, 255, 255, 100)
    outputChatBox(string.format("Ten xe: %s (ID: %d)", vehName, vehModel), player, 255, 255, 255)
    outputChatBox(string.format("Chu so huu: %s", owner), player, 255, 255, 255)
    outputChatBox(string.format("Nhien lieu: %d%%", fuel), player, 255, 255, 255)
    outputChatBox(string.format("Do ben: %.1f%%", health / 10), player, 255, 255, 255)
    outputChatBox(string.format("Khoa: %s", locked), player, 255, 255, 255)
