...
addCommandHandler("createprop", function(player, cmd, propType, propName, price)
    ...
    outputChatBox(string.format("✅ Da tao property '%s' (ID: %d, Type: %s, Price: $%d)", propName, propID, propType,
        propPrice), player, 0, 255, 0)
    outputDebugString(string.format("[ADMIN] %s created property %d (%s) at %.1f,%.1f,%.1f", getPlayerName(player),
        propID, propName, x, y, z))
end)
...
addCommandHandler("buyprop", function(player)
    ...
    outputChatBox(string.format("✅ Da mua property '%s' voi gia $%d!", propertyData.name, propertyData.price), player,
        0, 255, 0)
    outputChatBox(string.format("🏠 %s da mua property '%s'", getPlayerName(player), propertyData.name), root, 255,
        255, 0)
end)
...
addCommandHandler("sellprop", function(player, cmd, targetName, price)
    ...
    outputChatBox(string.format("📨 Da gui offer ban property '%s' cho %s voi gia $%d", propertyData.name,
        getPlayerName(targetPlayer), sellPrice), player, 0, 255, 0)
    outputChatBox(string.format(
        "📨 %s muon ban property '%s' cho ban voi gia $%d. Su dung /acceptprop hoac /declineprop",
        getPlayerName(player), propertyData.name, sellPrice), targetPlayer, 255, 255, 0)
end)
...
addCommandHandler("acceptprop", function(player)
    ...
    if seller then
        ...
        outputChatBox(string.format("💰 Da ban property cho %s voi gia $%d", getPlayerName(player), offer.price),
            seller, 0, 255, 0)
    end

    removeElementData(player, "propOffer")
    outputChatBox(string.format("✅ Da mua property '%s' voi gia $%d!", offer.propertyName, offer.price), player, 0,
        255, 0)
end)
...
addCommandHandler("rent", function(player, cmd, action, ...)
    ...
    if action == "set" then
        ...
        if rentPrice == 0 then
            ...
            outputChatBox(string.format("🏠 Da set gia thue $%d/day cho property '%s'", rentPrice, propertyData.name),
                player, 0, 255, 0)
        end
    ...
    elseif action == "property" then
        ...
        if (playerData.money or 0) < propertyData.rent then
            outputChatBox(string.format("❌ Ban can $%d de thue property nay.", propertyData.rent), player, 255, 100,
                100)
            return
        end
        ...
        outputChatBox(string.format("🏠 Da thue property '%s' voi gia $%d/day", propertyData.name, propertyData.rent),
            player, 0, 255, 0)

        -- Notify owner
        local owner = getPlayerFromName(propertyData.owner)
        if owner then
            outputChatBox(string.format("🏠 %s da thue property '%s' cua ban", getPlayerName(player),
                propertyData.name), owner, 255, 255, 0)
        end
    ...
    elseif action == "pay" then
        ...
        outputChatBox(string.format("💰 Da tra tien thue $%d. Han su dung gia han 24h", propertyData.rent), player, 0,
            255, 0)
    ...
    elseif action == "leave" then
        ...
        local owner = getPlayerFromName(propertyData.owner)
        if owner then
            outputChatBox(string.format("🏠 %s da roi khoi property '%s'", getPlayerName(player), propertyData.name),
                owner, 255, 255, 100)
        end
    end
end)
...
addCommandHandler("myprops", function(player)
    ...
    if #ownedProps > 0 then
        ...
        for _, prop in ipairs(ownedProps) do
            outputChatBox(string.format("• ID %d: %s (%s) - $%d", prop.id, prop.name, prop.type, prop.price), player,
                255, 255, 255)
        end
    end
    ...
    if #rentedProps > 0 then
        ...
        for _, prop in ipairs(rentedProps) do
            ...
            outputChatBox(string.format("• %s (%s) - %dh left", prop.name, prop.type, hoursLeft), player, 255, 255,
                255)
        end
    end
...
addCommandHandler("upgrade", function(player, cmd, upgradeType)
    ...
    outputChatBox(string.format("✅ Da upgrade '%s' cho property voi gia $%d", upgrades[upgradeType].name, cost),
        player, 0, 255, 0)
end)
