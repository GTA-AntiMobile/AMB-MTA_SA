...
addCommandHandler("families", function(player)
    ...
    for name, data in pairs(families) do
        ...
        outputChatBox(COLOR_WHITE .. count .. ". " .. name .. " (Leader: " .. data.leader .. ", Members: " .. memberCount .. ")", player)
    end
    if count == 0 then
...
-- Family command: /fam
addCommandHandler("fam", function(player, cmd, ...)
...
addCommandHandler("finvite", function(player, cmd, targetName)
    local family = getPlayerFamily(player)
    if not family then
        outputChatBox(COLOR_RED .. "You are not in a family!", player)
        return
    ...
    if invitePlayerToFamily(family, getPlayerName(target), getPlayerName(player)) then
        ...
        outputChatBox(COLOR_GREEN .. "You have been invited to " .. family .. " by " .. getPlayerName(player) .. ".", target)
        outputChatBox(COLOR_GREEN .. "Welcome to the family!", target)
        
        -- Notify family
        local message = COLOR_YELLOW .. getPlayerName(target) .. " has joined the family (invited by " .. getPlayerName(player) .. ")"
        for memberName, _ in pairs(families[family].members) do
            local member = getPlayerFromName(memberName)
...
addCommandHandler("fkick", function(player, cmd, targetName)
    local family = getPlayerFamily(player)
    if not family then
        outputChatBox(COLOR_RED .. "You are not in a family!", player)
        return
    ...
    if not families[family].members[targetName] then
        outputChatBox(COLOR_RED .. "This player is not in your family!", player)
        return
    end
    
    if removePlayerFromFamily(family, targetName, getPlayerName(player)) then
        outputChatBox(COLOR_GREEN .. "You kicked " .. targetName .. " from the family.", player)
        if target then
            outputChatBox(COLOR_RED .. "You have been kicked from " .. family .. " by " .. getPlayerName(player) .. ".", target)
        end
        
        -- Notify family
        local message = COLOR_YELLOW .. targetName .. " has been kicked from the family by " .. getPlayerName(player)
        for memberName, _ in pairs(families[family].members) do
            local member = getPlayerFromName(memberName)
            if member and member ~= target then
                outputChatBox(message, member)
...
addCommandHandler("fpromote", function(player, cmd, targetName)
    local family = getPlayerFamily(player)
    if not family then
        outputChatBox(COLOR_RED .. "You are not in a family!", player)
        return
    ...
    local currentRank = families[family].members[targetName]
    if currentRank >= 4 then
        outputChatBox(COLOR_RED .. "This player is already at maximum rank!", player)
        return
    end
    
    families[family].members[targetName] = currentRank + 1
    if target then
        setElementData(target, "familyRank", currentRank + 1)
    end
    
    local newRankName = familyRanks[currentRank + 1]
    outputChatBox(COLOR_GREEN .. "You promoted " .. targetName .. " to " .. newRankName .. ".", player)
    if target then
        outputChatBox(COLOR_GREEN .. "You have been promoted to " .. newRankName .. " by " .. getPlayerName(player) .. ".", target)
    end
end)

-- Family command: /fdemote
addCommandHandler("fdemote", function(player, cmd, targetName)
    local family = getPlayerFamily(player)
    if not family then
        outputChatBox(COLOR_RED .. "You are not in a family!", player)
        return
    ...
    local newRankName = familyRanks[currentRank - 1]
    outputChatBox(COLOR_GREEN .. "You demoted " .. targetName .. " to " .. newRankName .. ".", player)
    if target then
        outputChatBox(COLOR_ORANGE .. "You have been demoted to " .. newRankName .. " by " .. getPlayerName(player) .. ".", target)
    end
end)

-- Family command: /fwar
addCommandHandler("fwar", function(player, cmd, targetFamily)
    local family = getPlayerFamily(player)
    if not family then
        outputChatBox(COLOR_RED .. "You are not in a family!", player)
        return
