...
addCommandHandler("mute", function(player, _, playerIdOrName, time, ...)
    ...
    outputChatBox(
    "Ban da bi mute boi admin " .. getPlayerName(player) .. " trong " .. muteTime .. " phut. Ly do: " .. reason, target,
        255, 255, 0)
...
addEventHandler("onResourceStart", resourceRoot, function()
    ...
    for _, player in ipairs(getElementsByType("player")) do
        ...
        if playerID >= 0 then
            ...
            outputDebugString("[PLAYER] Reassigned ID " ..
            playerID .. " to " .. getPlayerName(player) .. " after restart")
        else
            outputDebugString("[PLAYER] Server full during restart! Cannot assign ID to " .. getPlayerName(player), 2)
        end
    end

    outputDebugString("[PLAYER] Player ID initialization complete. " ..
    #getElementsByType("player") .. " players assigned IDs.")
end)
...
addEventHandler("onPlayerQuit", root, function()
    ...
    elseif playerID then
        outputDebugString("[PLAYER] Warning: Player " ..
        getPlayerName(source) .. " had ID " .. playerID .. " but playerSlots table not available (resource stopping?)")
    end
end)
