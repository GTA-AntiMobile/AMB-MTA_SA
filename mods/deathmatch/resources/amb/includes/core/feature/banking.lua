...
addCommandHandler("ruttien", function(player, _, amount)
    ...
    if bankAccount.balance < totalCost then
        outputChatBox("So du khong du! Can: $" .. totalCost .. " (bao gom phi $" .. bankingSystem.withdrawFee .. ")",
            player, 255, 0, 0)
        return
    end
...
addCommandHandler("chuyentien", function(player, _, playerIdOrName, amount)
    ...
    if senderAccount.balance < totalCost then
        outputChatBox("So du khong du! Can: $" .. totalCost .. " (bao gom phi $" .. bankingSystem.transferFee .. ")",
            player, 255, 0, 0)
        return
    end
    ...
    outputChatBox("Da chuyen $" .. transferAmount .. " cho " .. getPlayerName(target) .. " (phi $" ..
                      bankingSystem.transferFee .. ")", player, 0, 255, 0)
...
addCommandHandler("aguitien", function(player, _, playerIdOrName, amount)
    ...
    outputChatBox("Admin " .. getPlayerName(player) .. " da them $" .. addAmount .. " vao tai khoan cua ban", target,
        255, 255, 0)
end)
...
addCommandHandler("aruttien", function(player, _, playerIdOrName, amount)
    ...
    outputChatBox("Admin " .. getPlayerName(player) .. " da tru $" .. removeAmount .. " tu tai khoan cua ban", target,
        255, 255, 0)
end)
...
addCommandHandler("ataikhoan", function(player, _, playerIdOrName)
    ...
    for i = #targetAccount.transactions, math.max(1, #targetAccount.transactions - 4), -1 do
        ...
        elseif transaction.type == "withdraw" then
            outputChatBox("[" .. timeStr .. "] Rut: -$" .. transaction.amount .. " (phi: $" .. transaction.fee .. ")",
                player, 255, 255, 0)
        elseif transaction.type == "transfer_out" then
            outputChatBox("[" .. timeStr .. "] Chuyen cho " .. transaction.target .. ": -$" .. transaction.amount,
                player, 255, 0, 0)
        elseif transaction.type == "transfer_in" then
            outputChatBox("[" .. timeStr .. "] Nhan tu " .. transaction.sender .. ": +$" .. transaction.amount, player,
                0, 255, 0)
        end
