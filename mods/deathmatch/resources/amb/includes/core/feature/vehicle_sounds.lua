...
addCommandHandler("testhorn", function(player, cmd, modelID)
...
addCommandHandler("testengine", function(player, cmd, modelID)
    local vehicle = getPedOccupiedVehicle(player)
    if not vehicle then
        outputChatBox("Ban can o trong xe!", player, 255, 0, 0)
        return
...
addCommandHandler("addsound", function(player, cmd, soundType, modelID, soundFile)
