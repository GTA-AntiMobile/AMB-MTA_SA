...
addEventHandler("onPlayerMarkerHit", root, function(marker, matchingDimension)
    if not activitySystem.parkour[source] then
        return
    end
    ...
    if marker == parkourData.markers[currentCP] then
        if currentCP == #activitySystem.activities.parkour.checkpoints then
            ...
            outputChatBox("Checkpoint " .. currentCP .. " hoan thanh! Den checkpoint " .. parkourData.currentCP, source,
                255, 255, 0)
        end
    end
end)
...
addCommandHandler("searchcar", function(player, _, playerIdOrName)
    ...
    if #foundItems > 0 then
        ...
        outputChatBox("Canh sat " .. getPlayerName(player) .. " da tim thay vat bat hop phap trong xe cua ban!", target,
            255, 0, 0)
    else
