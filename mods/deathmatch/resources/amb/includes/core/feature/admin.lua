...
addCommandHandler("osuspend", function(player, _, playerIdOrName, hours, ...)
    ...
    outputChatBox(getPlayerName(target) .. " da bi suspend " .. suspendHours .. " gio boi " .. getPlayerName(player), root, 255, 255, 0)
...
addCommandHandler("permaban", function(player, _, playerIdOrName, ...)
    ...
    outputChatBox(getPlayerName(target) .. " was permanently banned boi " .. getPlayerName(player) .. ", ly do: " .. reason, root, 255, 0, 0)
...
addCommandHandler("banaccount", function(player, _, accountName, ...)
    ...
    for _, p in ipairs(getElementsByType("player")) do
        if getElementData(p, "account.name") == accountName then
            outputChatBox(getPlayerName(p) .. " was banned boi " .. getPlayerName(player) .. ", ly do: " .. reason, root, 255, 0, 0)
...
addCommandHandler("fine", function(player, _, playerIdOrName, amount, ...)
    ...
    outputChatBox(getPlayerName(target) .. " da tru $" .. fineAmount .. " boi " .. getPlayerName(player) .. ", ly do: " .. reason, root, 255, 255, 0)
...
addCommandHandler("sfine", function(player, _, playerIdOrName, amount, ...)
    ...
    outputChatBox(getPlayerName(target) .. " was silent fined $" .. fineAmount .. " boi " .. getPlayerName(player) .. ", ly do: " .. reason, player, 255, 255, 0)
    outputChatBox("Ban bi phat $" .. fineAmount .. ". Ly do: " .. reason, target, 255, 0, 0)
end)
...
addCommandHandler("giveweapon", function(player, _, playerIdOrName, weaponId, ammo)
    ...
    if not weaponId or weaponId < 1 or weaponId > 46 then
        outputChatBox("Weapon ID khong hop le! (1-46)", player, 255, 0, 0)
        return
    end
    
    giveWeapon(target, weaponId, ammo)
    outputChatBox("Da give weapon " .. weaponId .. " cho " .. getPlayerName(target), player, 0, 255, 0)
