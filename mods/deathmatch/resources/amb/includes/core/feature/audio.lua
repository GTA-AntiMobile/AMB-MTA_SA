...
local audioSystem = {
    ...
    stations = {{
        ...
        url = "http://radio.ls.com/stream"
    }, {
        name = "K-DST",
        url = "http://kdst.radio.com/stream"
    }, {
        name = "Radio X",
        url = "http://radiox.com/stream"
    }, {
        name = "CSR 103.9",
        url = "http://csr1039.com/stream"
    }, {
        name = "K-JAH West",
        url = "http://kjah.com/stream"
    }}
}
    ...
    if audioSystem.boomboxes[player] then
        ...
        for _, nearbyPlayer in ipairs(getElementsByType("player")) do
            if nearbyPlayer ~= player then
                ...
                if distance <= 30 then
                    outputChatBox(getPlayerName(player) .. " dang phat nhac: " .. station.name, nearbyPlayer, 200, 200,
                        200)
                end
            end
        end
