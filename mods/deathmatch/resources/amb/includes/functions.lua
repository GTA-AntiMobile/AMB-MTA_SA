...
function isPlayerAdmin(player, requiredLevel)
    ...
    outputDebugString("[ADMIN] Access " .. (result and "GRANTED" or "DENIED") .. " (Level: " .. adminLevel ..
                          " vs Required: " .. requiredLevel .. ")")
    return result
end
...
function hasPermission(player, permission, level)
    ...
    outputDebugString("[PERMISSION] Player " .. getPlayerName(player) .. " has admin level: " .. adminLevel ..
                          ", checking " .. permission .. " level " .. level)
    ...
    elseif permission == "police" then
        ...
        return job == "police" or faction == "LSPD" or faction == "SFPD" or faction == "LVPD" or
                   hasPermission(player, "admin", ADMIN_LEVELS.HELPER)
...
function logVehiclePurchase(player, vehicle, dealership)
    ...
    print("[VEHICLE LOG] " .. getPlayerName(player) .. " purchased " .. vehicle.name .. " for $" ..
...
function isValidPassword(password)
    if not password or #password < 4 or #password > 50 then
        return false
    end
    return true
end
